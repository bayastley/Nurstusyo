...
export async function getUser(userId: string): Promise<DbUser | null> {
  if (!isSupabaseConfigured()) return null;
  const rows = await request<DbUser[]>(`/rest/v1/nur_users?id=eq.${encodeURIComponent(userId)}&select=*`, { method: "GET" });
  return rows[0] ?? null;
}

export async function ensureWallet(userId: string): Promise<DbWallet | null> {
  if (!isSupabaseConfigured()) return null;
  const existing = await request<DbWallet[]>(`/rest/v1/nur_wallets?user_id=eq.${encodeURIComponent(userId)}&select=*`, { method: "GET" });
...
export async function getWallet(userId: string): Promise<DbWallet | null> {
  if (!isSupabaseConfigured()) return null;
  const rows = await request<DbWallet[]>(`/rest/v1/nur_wallets?user_id=eq.${encodeURIComponent(userId)}&select=*`, { method: "GET" });
  return rows[0] ?? null;
}
...
export async function grantPurchasedTokens(userId: string, amount: number): Promise<DbWallet | null> {
  ...
  const rows = await request<DbWallet[]>(`/rest/v1/nur_wallets?user_id=eq.${encodeURIComponent(userId)}`, {
    method: "PATCH",
    body: JSON.stringify({ purchased_jeton: wallet.purchased_jeton + Math.max(0, Math.floor(amount)), updated_at: new Date().toISOString() }),
  });
  return rows[0] ?? null;
}
...
export async function getOrder(orderId: string): Promise<DbOrder | null> {
  if (!isSupabaseConfigured()) return null;
  const rows = await request<DbOrder[]>(`/rest/v1/nur_orders?id=eq.${encodeURIComponent(orderId)}&select=*`, { method: "GET" });
  return rows[0] ?? null;
}
...
export async function logAdminAction(input: { adminId: string; adminEmail: string; action: string; target?: string }): Promise<void> {
  ...
  await request("/rest/v1/nur_admin_audit_logs", {
    method: "POST",
    body: JSON.stringify({ admin_id: input.adminId, admin_email: input.adminEmail, action: input.action, target: input.target || "" }),
  });
}
...
export async function getUserByEmail(email: string): Promise<DbUser | null> {
  if (!isSupabaseConfigured()) return null;
  const rows = await request<DbUser[]>(`/rest/v1/nur_users?email=eq.${encodeURIComponent(email.toLowerCase())}&select=*`, { method: "GET" });
  return rows[0] ?? null;
}

export async function banUserInDb(input: { userId: string | null; email: string; reason: string; bannedBy: string; isAuto?: boolean }): Promise<void> {
  if (!isSupabaseConfigured()) return;
  await request("/rest/v1/nur_ban_logs", {
    method: "POST",
    body: JSON.stringify({
