// ════════════════════════════════════════════════════════
// titleData.desc.ts — Açıklama bölümü verileri
...
export const DESC_INTRO: Record<string, string[]> = {
  tr: [
    ...
    // ─── Genişletilmiş giriş havuzu (v2) ───
    "🌙 Gecenin huzuru: {S} Suresi {N}:{A}",
    "🌿 Ruhun gıdası: {S} {N}:{A}",
    "💎 Kalbe işleyen ayet: {S} {N}:{A}",
    "🕊️ Teselli bulacağın tilavet · {S} {N}:{A}",
    "🌾 Bereketli bir durak: {S} {N}:{A}",
    "💫 Aklına takılan soruların cevabı: {S} {N}:{A}",
    "🌊 Gönül okyanusundan bir damla: {S} {N}:{A}",
    "🤲 Duaların kabul vakti: {S} {N}:{A}",
    "🌅 Sabah tilaveti · {S} {N}:{A}",
    "🕯️ Kalbini aydınlat: {S} {N}:{A}",
    "📿 Zikrin en güzeli: {S} {N}:{A}",
    "🌟 Günün ayet-i kerîmesi: {S} {N}:{A}",
    "🎧 Kulaklıkla, kalbinle: {S} {N}:{A}",
    "🏔️ Sabrın sesi: {S} {N}:{A}",
    "💞 Sevdiklerine de gönder: {S} {N}:{A}",
  ],
  en: [
    "📌 {S} {N}:{A} — Noble Verse",
    "📖 Surah {S} · Verse {N}:{A}",
